...
def generate(no_of_cases, regex):
    ...
    input_prompt = f'You are a software QA expert, I want you to write a set of {no_of_cases} test cases to test if the regex is correctly witten for an entity with given requirement. The given requirements is must be a "{regex}" Please cover the edge cases and corner cases as much as possible while generating the test cases and segregate them in positive and negative classes. Generate the output in a format easy to display on a webpage such as a markdown table. Do not include Regex in the output. The output has to always be a markdown table with the columns being "Test Case" "Input" "Expected Result"'
